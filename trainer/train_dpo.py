...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    ...
    # Split chosen and rejected batches
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=4e-8, help="Initial learning rate (<=5e-8 recommended to avoid forgetting)")
    ...
    parser.add_argument('--max_seq_len', default=1024, type=int, help="Max sequence length in tokens (Chinese ~1.5-1.7 chars/token; English ~4-5 chars/token)")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-DPO", help="W&B project name")
    ...
    # ========== 5. Define policy and reference models ==========
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)
    Logger(f'Policy model total params: {sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # Init reference model (ref_model frozen)
    ref_model, _ = init_model(lm_config, args.from_weight, device=args.device)
    ref_model.eval()
    ref_model.requires_grad_(False)
    Logger(f'Reference model total params: {sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M')
